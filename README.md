...
### 区分基本数据类型与对象类型的原因
主要是基于程序性能的考量，基本类型定义定义的变量是存放在栈中，比如```int i = 5；```而```Integer j = new Integer（10）；```j则只是一个对象的引用，存放在栈中，而实际的数值10则是放在堆里，堆的读写速度远不及栈了。再有就是基本类型定义的变量创建和销毁很快，而类定义的变量还需要JVM去销毁。
